...
import org.sight.kiosk.enums.CODES;
...
public class PcscReader 
{
	...
	public boolean _connectionActive;
	...
	public PollingEvent getPollingEventHandler() { return this._pollingEventHandler; }
	public void setPollingEventHandler(PollingEvent pollingEventHandler) { this._pollingEventHandler = pollingEventHandler; }
	...
	public String[] listTerminals() throws CardException, Exception
	{
		try
		{
		...
		{
			if(ex.getCause().getMessage().equals("SCARD_E_SERVICE_STOPPED"))
			{
				establishContext();
				setCardTerminalList(getTerminalFactory().terminals().list());
			}
		...
		public int connectDirect(int terminalNumber, boolean isSetTerminalNumber) throws Exception
		{
			...
			{			
				if(ex.getCause().getMessage().equals("SCARD_E_SERVICE_STOPPED"))
				{
					...
					{
						setCardTerminalList(previousCardTerminals);
						throw new PcscException(CODES.SCARD_E_UNKNOWN_READER);
					}
	...
	{
		if (getActiveTerminal().isCardPresent() != true)
		{		    
